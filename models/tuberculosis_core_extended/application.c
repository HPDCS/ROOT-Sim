...
void ProcessEvent(unsigned int me, simtime_t now, int event_type,
	...
	switch (event_type) {
		...
		case GUY_LEAVE:
			guy_on_leave(*payload.agent, now, state);
			break;
...
void RestoreApproximated(void *ptr)
{
	...
	if (sicks > 0) reinitialize_missing_agents(sicks, SICK, region->now);
	if (infecteds > 0) reinitialize_missing_agents(infecteds, INFECTED, 0.0);
	if (treatments > 0) reinitialize_missing_agents(treatments, TREATMENT, 0.0);
	if (treateds > 0) reinitialize_missing_agents(treateds, TREATED, 0.0);
