...
typedef struct _revwin_mmap {
	...
	revwin *map[];			//! Array of pointers to the revwin state for each thread
} revwin_mmap;
...
extern void reverse_init(unsigned int num_threads, size_t revwin_size);
...
/**
 * Initializes locally a new reverse window and registers it
 * in the global reverse area.
 *
 * @param tid The ID of the calling thread
 */
extern void revwin_create(unsigned int tid);
 ...
 */
extern void revwin_free(void);


/**
 * Reset local reverse window
 */
extern void revwin_reset(void);
