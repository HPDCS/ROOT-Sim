...
extern void (*LP_change_group)(GLP_state **GROUPS_global, LP_state actual_lp);
void real_LP_change_group(GLP_state **GROUPS_global, LP_state actual_lp){
	...
	for(i=0 ; i<n_grp ; i++){
		...
		if(ECS_entry_temp->count_access > threshold_access){
			...
			actual_lp->current_group = i;

			//TODO MN
			//	Manage force checkpoint to store the new group's image.
